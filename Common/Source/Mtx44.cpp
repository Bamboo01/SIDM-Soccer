...
/*
I got down syndrome and I'm lazy get a 3x3 matrix

Edit: don't work nvm, wil make a 3x3 matrix someday
*/
void Mtx44::SetToInertiaTensor(Vector3 pointofcontact, float mass)
{
	float x = pointofcontact.x;
	float y = pointofcontact.y;
	float z = pointofcontact.z;

	*this = Mtx44(
		mass * ((y*y) + (z*z)),		-mass * x * y,						-mass * x * z,							0,

		-mass * x * y,				mass* ((x * x) + (z * z)),			-mass * y * z,							0,

		-mass * x * z,				-mass * y * z,						mass* ((x * x) + (y * y)),				0,
		0,							0,									0,										1
	);
}